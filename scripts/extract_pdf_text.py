...
SELF-CONTAINED: This script auto-installs its own dependencies (pdfplumber).
No manual pip install needed. Works standalone in any Python 3.7+ environment.
...
Exit codes:
    ...
    3  Missing dependency (could not install)
"""
...
def ensure_pdfplumber():
    try:
        ...
        try:
            ...
            print(
                f"ERROR: Could not install 'pdfplumber': {e}\n"
                "Try manually: pip install pdfplumber",
                file=sys.stderr,
            )
            sys.exit(3)
...
# ---------------------------------------------------------------------------
# Inline extraction (importable without CLI args)
# ---------------------------------------------------------------------------
def extract_from_path(pdf_path: str) -> str:
    """
    Convenience function for use when this module is imported rather than
    called as a CLI script. Auto-installs pdfplumber if needed.
    ...
    if not text.strip():
        raise RuntimeError("No text extracted. PDF may be scanned/image-based.")
    return text
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract text from a PDF file (auto-installs pdfplumber).",
