...
**IMPORTANT: Do NOT rely on external scripts or file paths. Use the following self-contained Python code block to extract text from the PDF directly in the sandbox at runtime.**

Upload the user's PDF to the sandbox, then execute this code using the `e2b_bash` tool:
...
**Execute this using e2b_bash like:**
```bash
pip install pdfplumber -q && python -c "
...
**Workflow:**
1. Use `e2b_file_upload` to upload the user's PDF asset into the sandbox
2. Use `e2b_bash` with the inline Python above to extract text — no external script file needed
3. Capture the stdout output as the raw extracted text
4. Proceed to summarize

If the user has pasted text directly or provided a text file, skip this step.

If extraction fails or the PDF is scanned/image-based, let the user know and suggest
they provide a text version or use OCR.
...
- **Script not found errors**: NEVER tell the user to upload the script. The extraction code is embedded above — always run it inline via e2b_bash. The skill is fully self-contained.
...
| Task | Tool to Use |
|---|---|
| Upload user's PDF to sandbox | `e2b_file_upload` |
| Extract text from PDF | `e2b_bash` with inline pdfplumber Python |
| Save summary as downloadable file | `create_artifact` + `e2b_file_download` |
...
**You**:
1. Use `e2b_file_upload` to upload `paper.pdf` into the sandbox
2. Use `e2b_bash` to run inline pdfplumber extraction — NO external script needed
3. Identify document type as Academic paper
4. Apply the Academic template
5. Output the structured summary
6. Offer to dive deeper
